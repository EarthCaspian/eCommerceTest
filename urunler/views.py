...
api_key = 'sandbox-XJWys9LeHXfgOBzrBZGfZaFIT1GcTy84'  # canlıya alındığında anahtarların hepsinin başındaki sandbox kısmı silinecek.
secret_key = 'sandbox-qjPv03AOLrCe4stPUOdaWknROGQci3Zm'
...
def payment(request):
    ...
    fiyat = odeme.total
    my_sepet = Sepet.objects.get(ekleyen = kullanici, odendiMi = False)
    sepetAdi = my_sepet.urun
    buyer={
        'id': 'BY789',
    ...
    basket_items=[
        {
            'id': str(sepetAdi.seri_no),
            'name': sepetAdi.isim,
            'category1': str(sepetAdi.kategori),
            'category2': str(sepetAdi.altKategori.first()),
            'itemType': 'PHYSICAL',
            'price': sepetAdi.fiyat
        },
        # {
        #     'id': 'BI102',
        #     'name': 'Game code',
    ...
    if request.method == 'POST':
        if request.user.is_authenticated:
            ...
            if Sepet.objects.filter(ekleyen = request.user, urun = urunum, odendiMi = False).exists():
                ...
                sepet.adet += int(adet)
                # sepet.total = urunum.fiyat * sepet.adet (model.py da yapıldı)
                sepet.save()
    ...
    if request.method == 'POST':
        ...
        if 'sil' in request.POST:
            ...
            if adet == '0':
                ...
                sepet.adet = adet
                # sepet.total = sepet.urun.fiyat * int(adet) (model.py da yapıldı)
                sepet.save()
                messages.success(request, 'Sepet güncellendi.')
                return redirect('sepetim')
